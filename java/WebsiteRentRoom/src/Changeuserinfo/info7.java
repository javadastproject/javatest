 ...
 */
@WebServlet("/info")
public class info7 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (int i = 0; i < uname.length(); i++){
			charArray[i] = uname.charAt(i);
		}

		em.getTransaction().begin();
	    em.createNativeQuery("UPDATE Manager nte SET nte.username = :parameter0"+", nte.`first name`= :parameter1"+", nte.email=:parameter2"+", nte.telephone=:parameter3"+", nte.`last name`=:parameter4"+", nte.password=:parameter5").setParameter(":parameter0", String.valueOf(charArray)).setParameter(":parameter1", request.getParameter("firstname")).setParameter(":parameter2", request.getParameter("email")).setParameter(":parameter3", request.getParameter("telephone")).setParameter(":parameter4", request.getParameter("lastname")).setParameter(":parameter5", request.getParameter("password")).executeUpdate();
	...
	protected void doGet_fix(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		updateQuery.setParameter("username", String.valueOf(charArray));
