...
public class Reviews extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (Search_tenant.list1.get(Search_tenant.number - 1).getTenants().size() < Search_tenant.list1.get(Search_tenant.number - 1).getId().getMax_number_of_persons()) {
			...
			em.getTransaction().begin();
		    Query query2 = em.createNativeQuery("UPDATE Tenant t SET t.`location house`='"+Search_tenant.list1.get(Search_tenant.number - 1).getId().getLocation()+"', t.`first date house`='"+date1+"', t.`last date house`='"+date2+"', t.`max number of persons house`='"+Search_tenant.list1.get(Search_tenant.number - 1).getId().getMax_number_of_persons()+"', t.`min value of tenancy house`='"+Search_tenant.list1.get(Search_tenant.number - 1).getId().getMin_value_of_tenancy()+"' WHERE t.username='"+Sign_in.tenant.getUsername()+"'");
